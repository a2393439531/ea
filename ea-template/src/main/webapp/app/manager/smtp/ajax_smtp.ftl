   ...
   <table   class="table  table-condensed">
		...
		<#list rhs["dataList"]?sort_by("sortNob") as x>
			<tr>
				...
				<td>
					<input type="text" value='${x.name?if_exists}' onchange="javascript:action_smtp('update.do','column=name&id=${x.id?if_exists}&columnValue='+escape(encodeURIComponent(this.value)))" />  
				</td>
				<td>
					<select style="WIDTH: 50px"   onchange="javascript:action_smtp('update.do','column=sex&id=${x.id?if_exists}&columnValue='+this.value)">
				...
				<td>
					<input type="text" value='${x.birthDate?if_exists}' onchange="javascript:action_smtp('update.do','column=birthDate&id=${x.id?if_exists}&columnValue='+this.value)" onClick="WdatePicker()"/>
				</td>
				<td>
