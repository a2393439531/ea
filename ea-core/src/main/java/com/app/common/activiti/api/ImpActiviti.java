...
/*		List<Task> tasklistcondition = taskService.createTaskQuery()
				.taskAssignee(account).orderByTaskCreateTime().desc()
				.listPage((int)p.getStart() - 1, p.getMaxSize());
	...
	//add by hb for search candidate task  20140126 start
	@Override
	public Map<String, Object> getCandidatedOaTaskListByAccount(String account,
			int page, int maxSize) {
		Long count = taskService.createTaskQuery().taskCandidateUser(account).count();
		...
		List<Task> tasklist = taskService.createTaskQuery()
				.taskCandidateUser(account).orderByTaskCreateTime().desc()
				.listPage((int)p.getStart() - 1, p.getMaxSize());	
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("pagination", p);
	...
	@Override
	public Map<String, Object> getCandidatedOaTaskListByGroup(List<String> group,
			int page, int maxSize) {
		Long count = taskService.createTaskQuery().taskCandidateGroupIn(group).count();
		...
		List<Task> tasklist = taskService.createTaskQuery()
				.taskCandidateGroupIn(group).orderByTaskCreateTime().desc()
				.listPage((int)p.getStart() - 1, p.getMaxSize());	
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("pagination", p);
   ...
   //old
	...
	@Override
	public String startProcessAssignee(String processKey, String formId,  String initiator, String assignee) {
	...
	@Override
	public String startProcessAssigneeVar(String processKey, String formId, String initiator, String assignee, Map<String, Object> map) {
		Map<String, Object> var = new HashMap<String, Object>();
		var.put("formId", formId); // 业务ID
		var.put("assignee", assignee); // 下一环节处理人 
		var.put("initiator",initiator); // 任务发起人
	...
	/**
	 启动任务
	 * */
	public String startProcessNoAssigee(String processKey, String formId,String initiator) {
		Map<String, Object> var = new HashMap<String, Object>();
		var.put("formId", formId); // 业务ID
		var.put("initiator",initiator); // 任务发起人
		var.put("processKey", processKey); // 流程key
...
//	}
	...
	public List<OaTask> taskToOaTask(List<Task> tasklist) {
		...
		for (Task task : tasklist) {
			...
			String assignee = (String)getVariableByTaskId(task.getId(), "assignee");
			oaTask.setInitiator(initiator);
			oaTask.setAssignee(assignee);
	...
	public String startProcessVarNOAssigee(String processKey, String formId,
			String initiator,  Map<String, Object> map) {
		Map<String, Object> var = new HashMap<String, Object>();
		var.put("formId", formId); // 业务ID
		var.put("initiator",initiator); // 任务发起人
		var.put("processKey", processKey); // 流程key
	...
	//add by hb start
	@Override
	public List<OaTask> getAssigneeTaskListByAccount(String account) {
		List<Task> candidateTaskList = taskService.createTaskQuery().taskAssignee(account).list();
		return taskToOaTask(candidateTaskList);
	}
	//add by hb end
